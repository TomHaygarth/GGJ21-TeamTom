...
namespace Dislectek
{
    ...
    public class TTS_Interface : MonoBehaviour
    {
        ...
        static EventSystem eventSystem_;
        bool canToggle = true;
        ...
        //toggleoptions
        GameObject inGameToggleTOE;
        GameObject inGameTogglePOA;
        ...
        {
            if (!singleton_)
                Instantiate(Resources.Load("Dislectek_Prefab"));
        }
        ...
        void Start()
        {         
            inGameToggleTOE = GameObject.Find("ToggleTalkOnExit");
            inGameTogglePOA = GameObject.Find("TogglePauseActive");
        }
        ...
        void Awake()
        {
            ...
            eventSystem_ = FindObjectOfType<EventSystem>();
            if (eventSystem_ == null)
            {
                GameObject GO = new GameObject("EventSystem", typeof(EventSystem), typeof(StandaloneInputModule));
                eventSystem_ = GO.GetComponent<EventSystem>();
            }

            eventSystem_.transform.SetParent(transform);
        ...
        void Update()
        {
            ...
            if (UIActive)
            {
                //check if mouse is on rect window
                bool onWindow = RectTransformUtility.RectangleContainsScreenPoint(windowRect, Input.mousePosition);
                // If mouse is clicked with right parameters
                if (!onWindow && !TTSUiManager.optionsActive && Input.GetMouseButtonDown(0))
                {
        ...
        public void DeactivateTTSUi()
        {
            ...
            //UIObject.gameObject.SetActive(false);
            if (TalkOnExit == false)
                TTSSystem.skipToEnd();//disable to continue playign after exit
